...
public class InvaderFleetSprite implements Sprite
{
    ...
    final private int fleetXspeed;
    final private int fleetYDrop;
    private AudioSample wompSample;
    private AudioSample wumpSample;

    private GameEventDispatcher dispatcher;
    private boolean wompLast;
     ...
     */
    public InvaderFleetSprite( Rectangle boundingBox, FleetStrategy strategy )
    {
        this.boundingBox = boundingBox;
        fleetBox = new Rectangle( boundingBox.getLocation() );
        controlPoint = boundingBox.getLocation();
        
        fleetYDrop = strategy.getYDrop();
        fleetXspeed = strategy.getXSpeed();
        ...
        try
        {
            wompSample = new AudioSample( this, "sounds/womp.wav" );
            wumpSample = new AudioSample( this, "sounds/womp2.wav" );
        }
        catch ( IOException e )
        {
            e.printStackTrace();
        ...
        try
        {
            ...
            for ( int invaderNumber = 0; invaderNumber < strategy.getInvaderCount(); invaderNumber++ )
            {
                InvaderSprite sprite = new InvaderSprite( invaderNumber % colCount, invaderNumber / colCount, new Dimension(
                        32, 32 ), boundingBox, invaderSheet );
        ...
        fleetBox.height = strategy.getInvaderCount() / 5 * 60 - 10;
    ...
    {
        if ( obj instanceof MissileSprite || obj instanceof BaseSprite || obj instanceof PlayerShipSprite )
        {
    ...
    public void update()
    {
        ...
        if ( !wompLast && fleetBox.x / 32 % 2 == 0 && wompSample.getState() == AudioState.DONE )
        {
            wompSample.play();
            wompLast = true;
        }
        else if ( wompLast && fleetBox.x / 32 % 2 == 1 && wumpSample.getState() == AudioState.DONE )
        {
            wumpSample.play();
            wompLast = false;
        }
